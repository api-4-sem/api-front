...
<select id="" style="
      ...
      width: 200px; 
  ">
    <option value="" disabled selected hidden>{{ nomeParceiro }}</option>
...
">
  ...
  export default class RastreioView extends Vue {
    ...
    try {
      // Faz a requisição para obter o nome do parceiro e o status
      const response = await axios.get<{ nome: string; status: boolean }>('/api/nome-do-parceiro');
